...
const { jsdocCache } = require("./performance-cache");
...
/**
 * Parse a JSDoc comment with caching.
 * Results are cached using the comment value as the key.
 *
 * @param {Object} comment - ESLint comment object
 * @returns {Object|null} Parsed JSDoc object or null if not valid JSDoc
 */
function parseJSDocCached(comment) {
  if (!isJSDocComment(comment)) {
    return null;
  }

  const key = comment.value;
  if (jsdocCache.has(key)) {
    return jsdocCache.get(key);
  }
