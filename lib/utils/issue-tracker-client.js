 ...
 * Logs a warning if environment variable is not set.
 *
 * @param {string} tokenConfig - Token value or "$ENV_VAR" reference
 * @param {string} [fieldName="token"] - Field name for warning message
 * @returns {string|null} Resolved token value
 */
function resolveToken(tokenConfig, fieldName = "token") {
  if (!tokenConfig) {
    return null;
  }
  if (tokenConfig.startsWith("$")) {
    ...
    if (!value) {
      // Log warning to help users diagnose configuration issues
      console.warn(
        `[lint-my-lines] Warning: Environment variable "${envVar}" for ${fieldName} is not set. ` +
...
class IssueTrackerClient {
   ...
   * Uses LRU eviction when cache reaches MAX_TICKET_CACHE.
   *
   * @param {string} ticketId - Ticket ID
  ...
  setCache(ticketId, result) {
    ...
    if (ticketCache.size >= MAX_TICKET_CACHE && !ticketCache.has(key)) {
      const firstKey = ticketCache.keys().next().value;
      ticketCache.delete(firstKey);
    }
