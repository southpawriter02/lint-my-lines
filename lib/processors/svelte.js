...
// Reset patterns before use to ensure consistent behavior
...
function extractTemplateSections(text) {
  ...
  // Reset pattern state for consistent behavior
  resetPatterns();

  // Remove script and style sections to get template content
  const scriptMatches = [...text.matchAll(SCRIPT_PATTERN)];
  const styleMatches = [...text.matchAll(STYLE_PATTERN)];
