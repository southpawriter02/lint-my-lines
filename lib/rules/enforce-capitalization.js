        ...
        schema: [
            {
                ...
                properties: {
                    ...
                    ignoreUrls: {
                        type: "boolean",
                        description: "Ignore comments starting with URLs (default: true)"
                    },
                    ignoreCodeBlocks: {
                        type: "boolean",
                        description: "Ignore markdown code blocks (default: true)"
                    },
                    ignoreRegex: {
                        type: "string",
                        description: "Custom regex pattern to strip before checking capitalization"
                    },
        ...
        return {
            Program() {
                ...
                for (const classified of classifiedComments) {
                    ...
                    let text = getCommentText(comment);

                    // Skip empty comments
                    if (!text) continue;

                    // v1.1.2: Skip if comment starts with URL
                    if (ignoreUrlsOpt && /^https?:\/\//.test(text)) continue;

                    // v1.1.2: Process text with ignore options before checking
                    let processedText = text;
                    if (ignoreCodeBlocksOpt) {
                        processedText = stripCodeBlocks(processedText);
                    }
                    if (ignoreRegexOpt) {
                        processedText = applyIgnoreRegex(processedText, ignoreRegexOpt);
                    }
                    ...
                    if (ignoreInlineCode && processedText.startsWith("`")) continue;
