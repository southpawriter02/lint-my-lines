...
/**
 * Get the comment text content (first meaningful line for block comments)
...
function findAdjacentNode(comment, sourceCode) {
    ...
    // For trailing comments (same line as code before)
    // For leading comments (code starts on next line or after on same line)
        ...
        return {
            Program() {
                ...
                comments.forEach(comment => {
                    ...
                    if (shouldSkipComment(text)) return;
                    ...
                    const isTrailing = sourceCode.getTokenBefore(comment) &&
                        sourceCode.getTokenBefore(comment).loc.end.line === commentLine;

                    if (isTrailing && !checkTrailing) return;
                    if (!isTrailing && !checkLeading) return;
