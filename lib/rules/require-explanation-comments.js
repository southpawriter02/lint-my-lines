...
const { classifyComment } = require("../utils/comment-utils");
const {
 ...
 */
function isMeaningfulComment(comment, commentContext = {}) {
 ...
 */
function hasMeaningfulLeadingComment(node, sourceCode, commentContext = {}) {
    ...
    for (const comment of comments) {
        ...
        if (nodeStartLine - commentEndLine <= 1) {
            if (isMeaningfulComment(comment, commentContext)) {
                return true;
            }
        }
    }
    ...
    create: function (context) {
        ...
        function enterNesting(node) {
            ...
            if (nestingStack.length > maxNesting && !reportedNodes.has(node)) {
                if (!hasMeaningfulLeadingComment(node, sourceCode, commentContext)) {
        ...
        if (requireFor.includes("regex")) {
            visitors.Literal = function (node) {
                if (node.regex && !shouldIgnore(node) && !reportedNodes.has(node)) {
                    ...
                    if (!hasMeaningfulLeadingComment(target, sourceCode, commentContext)) {
                        reportedNodes.add(node);
                        context.report({
                            node,
